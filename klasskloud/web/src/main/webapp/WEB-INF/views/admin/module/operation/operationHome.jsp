...
<script type="text/javascript">
	$(function() {
		...
		$('.btn-create').on('click',function(e) {
			if (validate()) {
				var postData = $('#operation-form').serializeArray();
				var formURL = $('#operation-form').attr("action");
				$.ajax({
					url : formURL,
					type : "POST",
					data : postData,
					success : function(data, textStatus, jqXHR) {
						if (data.status == false) {
							...
							$(location).attr('href',
									'${contextroot}/operation/default/home');
						}
					},
		...
		$('.btn-reset').on('click',function(e) {
			if($(this).text() == 'Cancel'){
				$('#operation-form').find('input[name="name"]').attr('value','');
				$('#operation-form').find('input[name="id"]').attr('value','');
				$('.box-primary').find('.box-title').find('i').removeClass('fa-pencil-square');
				$('.box-primary').find('.box-title').find('i').addClass('fa-plus-square');
				$('.box-primary').find('.box-title').find('span').text('Add Operation');
		...
		$('#example').on( 'click', 'a', function (e) {
			...
			if(op == 'ajaxDelete'){
	            $.confirm({
	                ...
	                confirm: function() {
	                	$.ajax({
							url : '${contextroot}/rest/operations/'+data.id,
							type : 'DELETE',
							success : function(data, textStatus, jqXHR) {
				...
				$.ajax({
					...
					success : function(data,textStatus,jqXHR) {
						if (data.status == true) {
							...
							$('.box-primary').find('.box-title').find('span').text('Edit Operation');
							$('#operation-form').find('input[name="name"]').attr('value',data.object.name);
							$('#operation-form').find('input[name="id"]').attr('value',data.object.id);
	...
	function fetchList(){
		$('#example').DataTable( {
			//"processing":true,
	        "ajax": "${contextroot}/rest/operations",
	        "columns": [
	            { "data": "id" },
	            { "data": "name" },
	            { "": "", "width": "5%"}
	...
	function validate(){
		var flag = true;
		var name = $('#operation-form').find('input[name="name"]').val();
		if (name.length == 0) {
			flag = false;
			$('#name').css('border', '1px solid #CD5C5C');
			$('#operation-form').find('#name').siblings('.error-block').text('Name is required field');
		} else {
			$('#name').css('border', '');
			$('#operation-form').find('#name').siblings('.error-block').text('');
		}
...
<div class="content-wrapper" style="min-height: 100%;">
	<section class="content-header">
		<h1>
			<i class="fa fa-th-list"></i>Manage<small>Operations</small>
		</h1>
		<ul class="breadcrumb">
	...
	<section class="content">
		<div class="box box-primary">
			<div class="box-header with-border">
				<h3 class="box-title">
					<i class="fa fa-plus-square"></i> Add Operations
				</h3>
			</div>
			
			<form id="operation-form" action="${contextroot}/rest/operations"
				method="post">
		...
		<div class="box box-info">
			<div class="box-header with-border">
				<h3 class="box-title">
					<i class="fa fa-search"></i> View List of Operations
				</h3>
			</div>
